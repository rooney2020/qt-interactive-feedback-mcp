...
| 主界面 | 设置页面 |
|:---:|:---:|
| ![主界面](screenshots/feedback-main.png) | ![设置](screenshots/settings.png) |
| AI 摘要（Markdown 渲染）、选项、输入框、底部开关 | 默认开关、超时配置、自定义追加文本、快捷回复 |

## ✨ 核心特性

### 🎨 深色主题 UI
- 完全重设计的深色主题界面（MCP 反馈助手风格）
- AI 工作摘要区域支持 **Markdown 渲染**（标题、粗体、代码、列表、链接，标题蓝色高亮）
- 预定义选项以复选框呈现，支持多选；选项过多时自动滚动（限高 200px）
- 全局 6px 细圆角滚动条，背景透明仅显示滑块

### 📋 粘贴去格式
- 反馈文本框粘贴内容时自动去除富文本格式，只保留纯文本
...
- **结束会话**：位于“提交反馈”按钮前，点击后会二次确认，确认后自动回复 `结束会话【结束会话】`
- **发送快捷键提示**：底部会根据当前设置显示发送提示，可在 `Ctrl+Enter` 发送和 `Enter` 发送之间切换

### ⚡ 快捷回复
- 在反馈窗口点击「⚡ 快捷回复」按钮，从预设列表中一键填入常用回复
- 设置页面支持添加/编辑/删除快捷回复
- 支持「选中后直接提交」模式，选择后自动提交反馈
- 数据持久化存储至本地文件

### ⚙️ 设置页面
- 点击底部栏或 Tab 栏右上角的 **⚙ 齿轮按钮** 打开设置
- Linux daemon 模式下支持**系统托盘图标**，右键菜单可随时进入设置、显示窗口或退出
- 可配置项：默认"使用中文"/"重新读取Rules"勾选状态、启动时检查更新、发送快捷键、超时自动回复内容、自定义追加文本、快捷回复管理
- 超时自动回复文案默认值为：`请重新发起feedback调用，禁止自动结束会话，必须等待用户的明确指令！`
- 时间设置项配置的是**倒计时自动回复时间**；保存后程序会自动把 `mcp.json` 里的 `timeout` 调大 1 到 10 分钟，避免 MCP 比倒计时更早超时
- 结束会话的二次确认弹窗使用与主界面一致的深色样式，不再使用系统默认确认框布局
...
>
> 如果你在设置页面里调整了倒计时自动回复时间，程序会自动把 `mcp.json` 中的 `timeout` 扩展为“倒计时时间 + 1~10 分钟缓冲”，避免 MCP 提前超时。
...
如果你在 VS Code / GitHub Copilot 工作流中使用这个 feedback MCP，仓库已经提供了一套可直接复制的示例资产：
