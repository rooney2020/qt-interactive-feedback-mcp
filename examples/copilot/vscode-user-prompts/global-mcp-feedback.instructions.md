---
description: 每次对话自动启用反馈交互机制，始终优先使用 interactive_feedback，连续失败3次后才降级到 AskQuestion
applyTo: "**"
---
...
- **不同会话必须使用不同的 `tab_id`**
- **禁止**直接复制文档示例中的 UUID，必须每次随机生成

### tab_title（显示标题）

- `tab_title` 应简短（≤15字），描述当前会话的核心任务
- 根据对话上下文推断任务名称，例如：`"日报填写"`、`"代码审查"`、`"Bug修复"`、`"功能开发"`
...
```json
// 注意：下方 tab_id 仅为格式示例，实际使用时必须随机生成新的 UUID，严禁直接复制！
{"message": "...", "tab_id": "<随机生成的UUID>", "tab_title": "日报填写", "predefined_options": [...]}
{"message": "...", "tab_id": "<同一会话使用相同UUID>", "tab_title": "日报填写", "predefined_options": [...]}
```
...
**interactive_feedback 连续失败 3 次（非心跳错误）→ 降级到 AskQuestion：**
1. 记录连续失败次数（心跳不算失败）
2. 达到 3 次后，使用 `AskQuestion` 完成交互
3. 在选项中提供「尝试重新连接 interactive_feedback」选项
4. 如果用户选择重连或下次交互时，先尝试 `interactive_feedback`，成功则重置失败计数

**AskQuestion 异常结束时 → 切换到 interactive_feedback：**
1. 当 `AskQuestion` 被跳过或异常返回后，立即尝试 `interactive_feedback`
2. 循环直到成功建立交互或用户手动发送消息

## 选项标识规则（必须遵守）

无论使用 `interactive_feedback` 还是 `AskQuestion`，给出预定义选项时**必须遵守以下规则**：

1. **标识结束选项**：如果某个选项会导致会话结束，必须在选项文本末尾标注 `【结束会话】`
   - 示例：`"先这样，结束会话 【结束会话】"`、`"完成，不需要其他操作 【结束会话】"`
2. **提供工具切换选项**：在选项列表中，**必须提供一个切换到另一种交互工具的选项**
   - 使用 `interactive_feedback` 时，提供选项：`"切换到 AskQuestion"`
   - 使用 `AskQuestion` 时，提供选项：`"切换到 Feedback"`
   - 此选项用于用户主动切换交互方式，不受失败次数限制
