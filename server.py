...
_IM_CONFIG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".im_config.json")


def _load_im_config() -> dict:
    """Load input method configuration generated by setup.sh."""
    if os.path.exists(_IM_CONFIG_PATH):
        try:
            with open(_IM_CONFIG_PATH) as f:
                return json.load(f)
        except Exception:
            pass
    return {}
...
def _apply_im_env(env: dict) -> None:
    """Apply input method environment variables (QT_IM_MODULE, XMODIFIERS, LD_PRELOAD)."""
    im_config = _load_im_config()
    im_module = im_config.get("im_module", "fcitx")
    env.setdefault("QT_IM_MODULE", im_module)
    env.setdefault("XMODIFIERS", f"@im={im_module}")

    ld_preload = im_config.get("ld_preload", "")
    if ld_preload and os.path.exists(ld_preload):
        existing = env.get("LD_PRELOAD", "")
        if ld_preload not in existing:
            env["LD_PRELOAD"] = f"{ld_preload}:{existing}" if existing else ld_preload
            _slog(f"LD_PRELOAD set to: {env['LD_PRELOAD']}")
